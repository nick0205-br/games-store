...
import { listarTodosJogos, listarTodosJogosPorNome, filtrarValorCem, filtrarValorCinquenta, filtrarValorDuzentos, filtrarValorFinal, filtraPlataformaPc, filtraPlataformaGB, filtraPlataformaPs5, filtraPlataformaPs4, filtraPlataformaPs3, filtraPlataformaPs2, filtraPlataformaPs1, filtraPlataformaXbox, filtraPlataformaSN, filtraPlataformaSW, filtraPlataformaWii, filtraPlataformaAtari, filtraGeneroAcao, filtraGeneroAventura, filtraGeneroSimulacao, filtraGeneroRPG, filtraGeneroCasual } from '../../../api/jogos'
    ...
    useEffect(() => {
        async function filtrar() {
            ...
            if (filtro === '1'){
                ...
                console.log(filtro)
                resp = await filtraPlataformaPs5()
                setJogos(resp)
            }else if(filtro === 'ps3'){
                console.log(filtro)
                resp = await filtraPlataformaPs3()
                setJogos(resp)
            }else if(filtro === 'ps2'){
                console.log(filtro)
                resp = await filtraPlataformaPs2()
                setJogos(resp)
            }else if(filtro === 'ps1'){
                console.log(filtro)
                resp = await filtraPlataformaPs1()
                setJogos(resp)
            }else if (filtro === 'xbox'){
                resp = await filtraPlataformaXbox()
                setJogos(resp)
            }else if(filtro === 'atari'){
                console.log(filtro)
                resp = await filtraPlataformaAtari()
                setJogos(resp)
            }else if(filtro === 'wii'){
                console.log(filtro)
                resp = await filtraPlataformaWii()
                setJogos(resp)
            }else if(filtro === 'switch'){
                console.log(filtro)
                resp = await filtraPlataformaSW()
                setJogos(resp)
            }else if(filtro === 'SN'){
                console.log(filtro)
                resp = await filtraPlataformaSN()
                setJogos(resp)
            }else if (filtro === 'acao'){
            ...
            else if (filtro === 'casual'){
                resp = await filtraGeneroCasual()
                setJogos(resp)
            }
